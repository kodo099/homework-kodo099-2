...
def poland_cases_by_date(day: int, month: int, year: int = 2020) -> int:
    ...
    return df.loc[df["Country/Region"]=="Poland"][f"{month}/{day}/20"].values[0]
...
def top5_countries_by_date(day: int, month: int, year: int = 2020) -> List[str]:
    ...
    # Your code goes here (remove pass)
    a=[" ", " "," "," "," "]
    a=df[["Province/State", "Country/Region", f"{month}/{day}/20"]].groupby("Country/Region").sum().sort_values(by=f"{month}/{day}/20").tail(5)
    return a
